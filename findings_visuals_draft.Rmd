...
list.of.packages <- c("openxlsx","tidyr","dplyr","stringr","RPostgreSQL","data.table", "janitor", ggplot, "kableExtra") #,"plyr"
...
### Male
```{r}
source("W:/Project/RDA Team/Region 5 State of the Child/R/male.R")
library(kableExtra)
...
av_plot <- 
  ...
  ggplot(df, aes(x= reorder(engvar, -value), y=value)) +     # set up manual fill using 'test', add "-" before value to order bars when MAX is best
  annotate("text", x=6.25, y = annotate_y, label = str_wrap(engtot_stat, width = 20), hjust=-0.33, vjust = 0.35, family= font_table_text, color = red) +  
  labs(title = "**<span style='font-size:20pt;'>Male Students (%)</span>**", subtitle = region, caption = "*American Indian and Alaska Native (AIAN), Native Hawaiian and Pacific Islander (NHPI)") +
  # rest of chart    
  geom_bar(stat="identity", position = position_dodge(0.7), show.legend = FALSE) +
  geom_col(fill = lightblue, color = darkblue) +         # all bars in "#355C7D"
  geom_hline(aes(yintercept = round(total_rate, 1)), color = red, size = .5) + 
...
### Female
```{r}
source("W:/Project/RDA Team/Region 5 State of the Child/R/female.R")
library(kableExtra)
...
av_plot <- 
  ...
  ggplot(df, aes(x= reorder(engvar, -value), y=value)) +     # set up manual fill using 'test', add "-" before value to order bars when MAX is best
  annotate("text", x=6.25, y = annotate_y, label = str_wrap(engtot_stat, width = 20), hjust=-0.33, vjust = 0.35, family= font_table_text, color = red) +  
  labs(title = "**<span style='font-size:20pt;'>Female Students (%)</span>**", subtitle = region, caption = "*American Indian and Alaska Native (AIAN), Native Hawaiian and Pacific Islander (NHPI)") +
  # rest of chart    
  geom_bar(stat="identity", position = position_dodge(0.7), show.legend = FALSE) +
  geom_col(fill = lightblue, color = darkblue) +         # all bars in "#355C7D"
  geom_hline(aes(yintercept = round(total_rate, 1)), color = red, size = .5) + 
...
## Student Population in Charter Schools
```{r}
source("W:/Project/RDA Team/Region 5 State of the Child/R/charter.R")
library(kableExtra)
...
av_plot <- 
  ...
  ggplot(df, aes(x= reorder(engvar, -value), y=value)) +     # set up manual fill using 'test', add "-" before value to order bars when MAX is best
  annotate("text", x=6.25, y = annotate_y, label = str_wrap(engtot_stat, width = 20), hjust=-0.33, vjust = 0.35, family= font_table_text, color = red) +  
  labs(title = "**<span style='font-size:20pt;'>Student Population in Charter Schools (%)</span>**", subtitle = region, caption = "*American Indian and Alaska Native (AIAN), Native Hawaiian and Pacific Islander (NHPI)") +
  # rest of chart    
  geom_bar(stat="identity", position = position_dodge(0.7), show.legend = FALSE) +
  geom_col(fill = lightblue, color = darkblue) +         # all bars in "#355C7D"
  geom_hline(aes(yintercept = round(total_rate, 1)), color = red, size = .5) + 
...
total_grade_only <- total_grade_only %>% filter(year=="2021-22")
