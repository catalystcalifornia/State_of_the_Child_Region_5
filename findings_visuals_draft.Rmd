...
source("W://Project//RDA Team//Region 5 State of the Child//R//passwords.R")
...
```
Source: [American Community Survey 2016-2020 5-year estimates calculated for Antelope Valley neighborhoods by Lucy Wilkerson](https://public.tableau.com/app/profile/luz3725/viz/2020CensusData-AVBESTSTARTREGION5/GRAPI). Note: All racial-ethnic groups in the table other than Latinx are non-Hispanic. Lancaster and Palmdale above are neighborhoods created by census tract aggregations as opposed to cities.
...
library(kableExtra)
av_table <- av_table %>% mutate(across(c(`2021-22 Count`), comma))%>% rename("Count"="2021-22 Count")
 
kbl(av_table) %>%
    kable_classic() %>%
  add_header_above(c("2021-22" = 1, "Foster" = 2))
 # kable_styling(bootstrap_options = c("striped", "bordered", "condensed"))
cde_foster <- av_table
...
# library(htmlwidgets)
# 
# title <- 'Non-Binary Counts by School District 2021-22 School Year'
# table_name <- "Nonbinary"
# html <- "Nonbinary.html"
# 
# 
# print_table <- function(x) {
# table_create <-   DT::datatable(x, extensions = 'FixedColumns', options = list(initComplete = JS(
#     "function(settings, json) {",
#     "$('body').css({'font-family': 'GothamBook'});",
#     "}"), 
#     dom = 't', ordering=F, searching=FALSE, scrollx = FALSE, fixedColumns = TRUE), caption = htmltools::tags$caption( style = 'caption-side: top; text-align: center; color:black;  font-size:200% ; ',title), rownames = FALSE, class = list(stripe = FALSE))
# 
# saveWidget(table_create, html)
# webshot(html, print(paste0(table_name,".png"))) # you can also export to pdf
# }
#  
# test_table2 <- print_table(av_table) 
# 
# # simplified version
# 
# table_create <- DT::datatable(av_table)
# ggplot(table_create + 
#          ggsave("mtcars.png"))
# html <- "table_create.html"
# saveWidget(table_create, html)
# webshot(html, "table_create.png") # you can also export to pdf
```
...
kbl(av_table) %>%
    kable_classic() %>%
  add_header_above(c("2021-22" = 1, "Socioeconomically Disadvantaged" = 2))
 # kable_styling(bootstrap_options = c("striped", "bordered", "condensed"))
...
The Hughes-Elizabeth Lakes Union Elementary School District and the Wilsona Elementary School District had the next highest rates, both being 2.3 times higher than the state average. For the Hughes-Elizabeth Lakes Union Elementary School District, the high rates for Latinx students were the highest (5.7%). In the case of the Wilsona Elementary School District, the high rates for Black (11.5%), Latinx (6.6%), and White students (6.2%) especially drove up the average. Eastside Union Elementary School District and Lancaster Elementary School District also had above average rates, being 2 and 1.2 times higher than the state average respectively. 

Regionally, students who identified as Non-Hispanic Two or More Races (3.2%) and Non-Hispanic Black (3.3%) and Non-Hispanic AIAN (2.7%) students had the highest rates. The bulk of the students experiencing homelessness in the Antelope Valley are in grades 1-8. Notably, student homelessness rates range between 15 to 20% of students. Second-grade students had the highest rates with 2 out of 10 experiencing homelessness. Given this information, interventions targeting current elementary and middle school students,  and Black, Latinx, AIAN, and Two or More Race students are pressing.
...
cde <- left_join(cde_foster, cde_socieconomically_disadvantaged, by="Geography") %>% left_join(cde_student_homelessness, by="Geography") %>% left_join(cde_disability, by="Geography") #%>% left_join(cde_ell, by="Geography")

# cde_table <- kbl(cde, col.names = c("Geography", "Count", "Rate (%)", "Count", "Rate (%)", "Count", "Rate (%)", "Count", "Rate (%)", "Count", "Rate (%)", "Count", "Rate (%)", "Count", "Rate (%)")) %>%
#     kable_classic() %>%
#   add_header_above(c("2021-22" = 1, "Foster" = 2, "Socioeconomically Disadvantaged" = 2, "Homeless" = 2, "Disability" = 2,"Migrant" = 2,  "ELL" = 2, "Non-Binary" = 2))%>%
#   save_kable(file = "cde_table.png")

# grouping variables for DT table creation
# sketch = htmltools::withTags(table(
#   class = 'display',
#   thead(
# # Define the grouping of your df
#     tr(
#       th(colspan = 1, '2021-22'),
#       th(colspan = 2, 'Foster'),
#       th(colspan = 2, 'Socioeconomically Disadvantaged'),
#       th(colspan = 2, 'Homeless'),
#       th(colspan = 2, 'Disability'),
# #      th(colspan = 2, 'ELL'),
#     )#,
...
Antelope Valley stakeholders have been clear about the need for additional resources, including health and mental health services, early childhood education, and parks and recreation.
...
According to the [Los Angeles County Health Survey](http://publichealth.lacounty.gov/ha/hasurveyintro.htm), **16.4% of Antelope Valley children report difficulty accessing medical care (highest among county Service Planning Areas or SPAs)**. The Antelope Valley has fewer health and medical professionals than other parts of the county and state. The area has a variety of medical and health professional shortage designations, including: 

* The Lake Los Angeles and Juniper Hills Little Rock Medical Services Study Areas (MSSAs) are Medically Underserved Areas[1](https://gis.data.ca.gov/datasets/CHHSAgency::medically-underserved-areas/explore?location=34.269244%2C-118.153917%2C9.58)
* The two MSSAs plus the Lancaster Central/Palmdale North Central and Palmdale Central MSSAs are Primary Care Health Professional Shortage Areas[1](https://gis.data.ca.gov/datasets/CHHSAgency::health-professional-shortage-area-primary-care/explore?location=34.384784%2C-117.925067%2C9.00)
* These four MSSAs plus the Acton/Ravenna, Desert View Highlands/Lancaster West/Palmdale Northwest/Quartz Hill, and Elizabeth Lake/Gorman/Green Valley/Lake Hughes/Leona Valley/Neenach MSSAs are mental health professional shortage areas[1](https://gis.data.ca.gov/datasets/CHHSAgency::health-professional-shortage-area-mental-health/explore?location=34.300335%2C-118.134212%2C9.00). This means that virtually the entire Antelope Valley is a mental health professional shortage areas.

Source: California Health and Human Services Geospatial Data portal for [Medically Underserved Areas](https://gis.data.ca.gov/datasets/CHHSAgency::medically-underserved-areas/explore?location=34.514402%2C-118.185118%2C9.58) and [Primary Care Health Professional](https://gis.data.ca.gov/datasets/CHHSAgency::health-professional-shortage-area-primary-care/explore?location=34.380830%2C-117.925067%2C9.00) and [Mental Health Professional](https://gis.data.ca.gov/datasets/CHHSAgency::health-professional-shortage-area-mental-health/explore?location=34.296386%2C-118.134212%2C9.00) Shortages Areas.

Health and mental health outcomes might suffer because of the lack of services. Medical care can be especially important for chronic conditions that need periodic treatment such as asthma. Antelope Valley adults are most likely to say they've ever seriously thought about committing suicide, 13% of the AV adult population, compared to 9.8% of LA County residents overall.[1](https://ask.chis.ucla.edu/)

18.8% of Antelope Valley children have been diagnosed with asthma at some point in their lives (highest among LA SPAs), meaning access to health care is especially important for the Antelope Valley population.
...
Foreclosure data was calculated using 2017 to partial 2022 data. In this time period there were a total of 546 foreclosures in the Antelope Valley Region. On average 1 out of 2,500 (or 3.8 out of 10,000) owner households in the AV faced foreclosures.

By rate, Lake Los Angeles had the most foreclosures, where 11.7 out of 10,000 owner households faced foreclosures, while West Antelope Valley had the least. Even the neighborhood with the smallest rate of foreclosures, Palmdale (2.8 out of 10,000 owner households), had more foreclosures than at the Los Angeles County or state where 1.8 per 10,000 owner households and 2 per 10,000 owner households faced foreclosures respectively.   

Although by rate, Lancaster and Palmdale did not have the highest rate of foreclosures, these two locations have the the greatest populations in the AV. Thus, they still had the most foreclosures by count, 239 and 181 respectively.

** Foreclosure Totals by Region and Neighborhood **
```{r}
source("W:/Project/RDA Team/Region 5 State of the Child/R/foreclosure.R")
...
```
Source: [DataQuick (2017 to partial 2022) purchased from DQNews](https://dqnews.com.cutestat.com/)


## Evictions

To calculate evictions, 2013-2017 data was used due to the limitations of available data. There have been 636 evictions in the AV between 2013-2017. Meaning 1 out of 50 households in the AV faced evictions during this time period. This is twice the rate of California and more than twice the rate of Los Angeles County where 1 in 100 households and 0.8 in 100 households face evictions respectively[1](https://www.racecounts.org/). By rate, Lake Los Angeles had the most evictions while Quartz Hill had the least. The neighborhoods of Lancaster (350 evictions) and Palmdale (226 evictions) had the most evictions by count. About half of all evictions during this time period were in Lancaster.

** Eviction Totals by Region and Neighborhood **
```{r}
...
According to AntiEvictionMap, in 2022 there were over 34,000 eviction filings in LA Superior Court, a 172% compared to the previous year, 2021. Evictions by the end of 2022 were already at pre-pandemic levels which further exacerbates the loss in jobs, income, and economic security caused by the Pandemic.

In February 2019 in the North Court District, Antelope Valley region, there were 135 eviction filing per month which steadily peaked to 198 eviction filings per month in June 2019. In June 2020 during he eviction moratorium, that number dwindled it its low in the AV of 7 eviction filings month and steadily rose to 61 filings per month by March 2021. While eviction filings were overall lower throughout the pandemic in Antelope Valley eviction trends mirrored those of the the rest of the county and the rates were rates were especially comparable to other outer areas of Los Angeles County. It was lower than the Central Court House District in the middle of the Los Angeles County urban area, which was the most impacted by evictions. Comparatively evictions in the AV were 10 times less than those of the most impacted region.

**Evictions During the Pandemic**

```{r}
source("W:/Project/RDA Team/Region 5 State of the Child/R/antievictionmap.R")
line_plot
```


Source: [AntiEvictionMap, 2022](https://antievictionmap.com/there-is-no-eviction-moratorium)

<div class="infogram-embed" data-id="3ec6cb57-dc01-490e-ab4f-5ae810640467" data-type="interactive" data-title="LA County Eviction Filings During the COVID-19 Pandemic"></div><script>!function(e,i,n,s){var t="InfogramEmbeds",d=e.getElementsByTagName("script")[0];if(window[t]&&window[t].initialized)window[t].process&&window[t].process();else if(!e.getElementById(n)){var o=e.createElement("script");o.async=1,o.id=n,o.src="https://e.infogram.com/js/dist/embed-loader-min.js",d.parentNode.insertBefore(o,d)}}(document,0,"infogram-async");</script><div style="padding:8px 0;font-family:Arial!important;font-size:13px!important;line-height:15px!important;text-align:center;border-top:1px solid #dadada;margin:0 30px"><a href="https://infogram.com/3ec6cb57-dc01-490e-ab4f-5ae810640467" style="color:#989898!important;text-decoration:none!important;" target="_blank">LA County Eviction Filings During the COVID-19 Pandemic</a><br><a href="https://infogram.com" style="color:#989898!important;text-decoration:none!important;" target="_blank" rel="nofollow">Infogram</a></div>
...
The average high school graduation rate in the Antelope Valley is 67%. In the AV, Latinx (66%) followed by Black (66%) students had the lowest graduation rates while Filipino (81%) and Asian (87%) students had the highest. The Acton-Agua Dulce Unified School District has the lowest graduation rate (35%). Notably in the Acton-Agua Dulce Unified School District Latinx, Black, Filipino, and students who identified as non-Hispanic Two or More Races all had under 50% graduation rates.  
...
In the Antelope Valley female Students graduation rates (72%) were 10 percentage points higher than their male counterparts (62%). About half of students with Disabilities (51%), English Language Learners (50%), and students in Foster Care (47%) were able to graduate in the AV. Students experiencing homelessness fared far worse with only 38% or 19 out of 50 students graduating from High School. According to the CDE students in the Migrant program had a much higher graduation rate with 89% graduating. However this number does not account for migrant students who are not registered in the Migrant Program. Students that the CDE considered Socioeconomically Disadvantaged (65%) around the total average (67%) which makes sense since the majority of the student population in the AV is considered to Socioeconomically Disadvantaged (82%).

**Highschool Graduation Counts and Rates (out of 100) by School District 2021-22 School Year**
```{r}
...
**Suspension Rates by Student Group, Antelope Valley School Districts, 2021-22**
```{r}
...
In the AV, female students scored proficiently 6 percentage points higher than their male counterparts. Economically Disadvantaged Students passed at a rate 3.6 percentage points under that of the AV average. Students experiencing homelessness, English Language Learners, and  students in Foster Care all scored proficiently 10 percentage points under the AV average. Students with disabilities had the lowest passing rate with only 9.4% getting proficient results.
...
# Combined 3rd Grade Table
```{r}
cde_math$Test <- "Math"
cde_ela$Test <- "ELA"
third_grade <- rbind(cde_math, cde_ela)
third_grade_table <- kbl(third_grade)%>%
    kable_classic() 
third_grade_table
```
...
## Hate Crimes
Between 2003 and 2020, 385 hate crimes were reported in the Antelope Valley, SPA 1, for a rate of 97 hate crimes per 100,000 people. Antelope Valley residents were subject to more hate crimes per capita than LA County residents, as LA County's hate crime rate was 67 hate crimes per 100,000 people during the same time period. Hate crimes against racial-ethnic groups made up 71% of AV hate crimes with 169 hate crimes perpetrated against Black AV residents, 61 hate crimes perpetrated against Latinx residents, and 45 hate crimes perpetrated against others.
```{r message=FALSE, warning=FALSE}
source("W:/Project/RDA Team/Region 5 State of the Child/R/hate_crimes.R")

library(kableExtra)
kbl(hate_crime_table) %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "condensed"))

```
Source: Catalyst California calculations of [Los Angeles County Human Resources Commission 2003-2020](https://egis-lacounty.hub.arcgis.com/datasets/lacounty::reported-hate-crimes/explore?location=34.215272%2C-118.173107%2C7.95) hate crimes and 2020 American Community Survey 5-year population estimates. Rates of hate crimes presented are the average number of hate crimes in the time period per 100,000 people. Antelope Valley in this table is the aggregation of census tracts in SPA 1.
