...
kbl(av_table) %>%
  # kable_styling(bootstrap_options = c("striped", "bordered", "condensed"))
    kable_classic() %>%
  add_header_above(c(" " = 1, "Non-Binary" = 2))
cde_nonbinary <- av_table
```
...
cde_nonbinary$Geography <- "Antelope Valley Best Start Region 5"
cde_nonbinary <- cde_nonbinary[!(cde_nonbinary$`School Year`=="2020-21"),]
cde_nonbinary <- cde_nonbinary %>% select(Geography, Count, `Rate (%)`)

cde <- left_join(cde_foster, cde_socieconomically_disadvantaged, by="Geography") %>% left_join(cde_student_homelessness, by="Geography") %>% left_join(cde_disability, by="Geography") %>% left_join(cde_migrant, by="Geography") %>% left_join(cde_ell, by="Geography") %>% left_join(cde_nonbinary)
