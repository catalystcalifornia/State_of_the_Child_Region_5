...
estot_stat <- paste0("Tasa General = ", min(av_df$Total), "%")   # Total line annotation
...
rent_plot <-
  ...
  annotate("text", x=6.25, y = annotate_y, label = str_wrap(engtot_stat, width = 20), hjust=-0.05,
           vjust = 38, family= font_table_text, color = darkblue, size = 20) +
  geom_text(aes(label = paste0(Rent_Burdened, "%")), family = font_table_text, hjust = -.1, size = 20) +       # format data labels, adjust hjust to avoid overlap w/ total line
...
rent_plot <- rent_plot + theme(
  ...
  plot.title = element_markdown(family = font_title, face = "bold", size = 64, hjust = 0, margin=margin(0,0,4,0)),
  plot.subtitle = element_markdown(family = font_title, face = "bold", size =52, hjust = 0, margin=margin(0,0,4,0)),
  plot.caption = element_text(family = font_caption, size = 40, hjust = 0, lineheight = 0.35),
...
#Spanish Version -----
av_df$NEIGHBORHOOD <- gsub("West Antelope Valley","Oeste Antelope Valley",av_df$NEIGHBORHOOD)
av_df$NEIGHBORHOOD <- gsub("East Antelope Valley","Este Antelope Valley",av_df$NEIGHBORHOOD)

rent_plot_esp <-
  ggplot(av_df, aes(x= reorder(NEIGHBORHOOD, Rent_Burdened), y=Rent_Burdened)) +     # set up manual fill using 'test', add "-" before value to order bars when MAX is best
  labs(title= "Casi tres de cada cinco inquilinos de Antelope Valley están<br>agobiados por el alquiler", 
       subtitle = "Porcentaje de hogares arrendatarios que gastan el 30% o más de sus ingresos en<br>alquiler", 
       caption = "Fuente: Cálculos de Lucy Wilkerson basados en estimaciones de 5 años de la Encuesta Comunitaria
Estadounidense 2016-2020.") + #, https://public.tableau.com/app/profile/luz3725/viz/2020CensusData-AVBESTSTARTREGION5/GRAPI. 
  geom_bar(stat="identity", position = position_dodge(0.7), show.legend = FALSE, fill = lightblue) +
  geom_hline(aes(yintercept = round(Total, 1)), color = darkblue, size = .5) +
  annotate("text", x=0, y = annotate_y, label = str_wrap(estot_stat, width =15), hjust=-0.05, 
           vjust = -1.5, family= font_table_text, color = darkblue, size = 20, lineheight = 0.3) +
  geom_text(aes(label = paste0(Rent_Burdened, "%")), family = font_table_text, hjust = -.1, size = 20) +       # format data labels, adjust hjust to avoid overlap w/ total line
  scale_x_discrete(labels = function(NEIGHBORHOOD) str_wrap(NEIGHBORHOOD, width = 15)) +            # wrap long labels
  theme_void()+
  xlab("") +
...
rent_plot_esp <- rent_plot_esp + theme(
  panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(),
  panel.border = element_blank(),
  panel.background = element_blank(),
  ...
  axis.text.y = element_text(size = 52, family= font_axis_label, lineheight = 0.3, hjust=0),
  axis.ticks = element_blank(),
  plot.title = element_markdown(family = font_title, face = "bold", size = 64, hjust = 0, margin=margin(0,0,4,0), lineheight = 0.3),
  plot.subtitle = element_markdown(family = font_title, face = "bold", size =52, hjust = 0, margin=margin(0,0,4,0), lineheight = 0.3),
...
ggsave(rent_plot_esp, filename="rent_burdened_chart_esp.png",
